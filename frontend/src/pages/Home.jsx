...
const TYPE_IC = { heizung:['#FCEBEB','#A32D2D',IconFlame], sanitaer:['#E6F1FB','#185FA5',IconDroplet], klima:['#FAEEDA','#854F0B',IconTool], wartung:['#F1EFE8','#5F5E5A',IconTool] }

export default function Home() {
  const { user } = useAuth()
  const navigate = useNavigate()
  const [projects, setProjects] = useState([])
  const [stats, setStats]       = useState({ projekte:0, heute:0, wiki:0 })
  const [loading, setLoading]   = useState(true)

  useEffect(() => {
    Promise.all([api.get('/projects'), api.get('/wiki')]).then(([p, w]) => {
      const aktive = p.data.filter(x => x.status === 'aktiv')
      setProjects(aktive.slice(0,4))
      setStats({ projekte: aktive.length, heute: aktive.length, wiki: w.data.length })
  ...
  return (
    <div style={{ padding:'14px 14px 0' }}>
      ...
      <div style={{ marginBottom:16 }}>
        <div style={{ fontSize:20, fontWeight:500 }}>{greeting}, {user?.name?.split(' ')[0]}</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:18 }}>
        {[
          ...
          { label:'Heute geplant',   val: stats.heute,    color:'#185FA5' },
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:18 }}>
        {[
          ...
          { icon: IconUpload,     label:'Datei hochladen',sub:'Dateien',       bg:'#E6F1FB', ic:'#185FA5', action:() => navigate('/projekte') },
          ...
          return (
            <button key={p.id} className="card" onClick={() => navigate(`/projekte/${p.id}`)}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ height:3, background:'#E1F5EE', borderRadius:2, marginTop:8 }}>
                  <div style={{ width:'50%', height:'100%', borderRadius:2, background:'#1D9E75' }} />
                </div>
              </div>
